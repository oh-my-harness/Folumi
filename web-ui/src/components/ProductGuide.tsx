...
const chineseCopy = {
  ...
  materials: {
    ...
    sourceTitle: '资料集或 Notebook：会话级资料源',
    sourceText: '适合让 Agent 在整个会话中按需检索一组内容。当前只能关联一个资料集或 Notebook。',
    mentionTitle: '@ 目标：精确引用已有内容',
    mentionText: '适合明确指定某条笔记，避免从整个资料源中猜测目标。',
  },
  knowledge: {
    title: '配置并使用资料',
    description: '资料页保存外部只读材料，并通过向量检索为会话提供可引用的依据。',
    steps: ['在“设置 > 嵌入模型”添加并测试嵌入配置。', '进入“资料”创建资料集并添加文档。', '回到聊天，打开输入框中的资料源下拉框，选择具体资料集。', '提问后检查回答中的引用；需要精确指定现有笔记时改用 @ 目标。'],
    embedding: '配置嵌入模型',
    open: '打开资料',
  },
  notebook: {
  ...
  memory: {
    ...
    open: '打开个性化',
    items: [
      { icon: <Brain size={18} />, title: '关于我', text: '管理事实、偏好、目标和对话连续性，并决定是否允许助手自动补充。' },
      { icon: <Bot size={18} />, title: '助手设定', text: '设置所有新会话共用的助手名称和行为说明。' },
    ],
  },
}
...
const englishCopy: typeof chineseCopy = {
  ...
  materials: {
    ...
    sourceTitle: 'Source collection or Notebook: conversation source',
    sourceText: 'Use it when the Agent should search a collection throughout the conversation. One source collection or Notebook can be associated at a time.',
  ...
  knowledge: {
    ...
    description: 'Sources keeps external read-only material and supplies cited context through vector retrieval.',
    steps: ['Add and test an embedding profile under Settings > Embedding.', 'Open Sources, create a source collection, and add documents.', 'Return to Chat and select that collection from the composer source menu.', 'Ask your question and inspect citations; use an @ target when you need one exact saved item.'],
  ...
  memory: {
    ...
    items: [
      { icon: <Brain size={18} />, title: 'About me', text: 'Manage facts, preferences, goals, and conversation continuity, including whether the Assistant may update them automatically.' },
      { icon: <Bot size={18} />, title: 'Assistant setup', text: 'Set the name and behavior instructions shared by new conversations.' },
    ],
  },
}
