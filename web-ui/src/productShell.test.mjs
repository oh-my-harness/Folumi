...
const notebookStyles = readFileSync(new URL('./index.css', import.meta.url), 'utf8')
...
test('Notebook keeps its IDE-like workspace instead of regressing to a flat note list', () => {
  ...
  assert.match(notebookLiveEditor, /new CrepeBuilder/)
  assert.match(notebookLiveEditor, /data-notebook-direct-editor="true"/)
  assert.match(notebookLiveEditor, /markdownUpdated/)
  assert.match(notebookLiveEditor, /userEditObservedRef/)
  assert.match(notebookLiveEditor, /onBeforeInput/)
  assert.match(notebookLiveEditor, /window\.setTimeout/)
  assert.doesNotMatch(notebookLiveEditor, /<textarea/)
  assert.doesNotMatch(notebookLiveEditor, /feature\/toolbar|addFeature\(toolbar\)/)
  assert.doesNotMatch(notebookLiveEditor, /feature\/block-edit|addFeature\(blockEdit\)/)
  assert.match(notebookStyles, /h1::before \{ content: '#'; \}/)
  assert.match(notebookStyles, /h6::before \{ content: '######'; \}/)
