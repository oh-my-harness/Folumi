...
| Memory | “保存的记忆”与“助手配置”可用；记忆默认关闭，只接受用户直接操作或逐次确认的助手写入，并支持有效期重确认和 JSON 导出。History Recall 也默认关闭，开启后通过 runtime 的可重建 Session 投影只读召回旧对话，不会自动写入 Saved Memory。 |
  ...
  -> Tauri shell
      ...
      -> managed tutor-web sidecar
          ...
          -> local product stores
               -> Sources / Notes / Sessions / Settings
```
