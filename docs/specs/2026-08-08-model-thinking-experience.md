...
Folumi 为每个 LLM 配置提供独立的默认思考强度，可选关闭、最少、低、中、高和极高。会话输入框同时提供当前会话的快捷档位；新会话继承所选模型的默认值，之后的修改只覆盖当前会话，切换模型时改用新模型的默认值。该配置使用 runtime 的 `ThinkingLevel`，由 provider adapter 翻译为对应接口参数；不在产品层重新实现供应商协议。
...
- 输入框工具栏显示当前档位，点击后可直接切换六档；弹层只保留档位和“当前会话”提示。
