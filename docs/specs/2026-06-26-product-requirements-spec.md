...
> consolidation system is retired. REQ-400 through REQ-406 describe the
> replacement product target and are not active until the new Memory proposal
> is accepted and implemented. Assistant Profile remains active.
...
- **REQ-402** User Memory shall hold stable preferences, explicit user facts,
  and long-term goals.
- **REQ-403** The same item model shall hold explicit commitments, open loops,
  and reusable working strategies; these categories shall not create hidden
  storage layers.
