...
#[cfg(any())]
mod notebook_transfer_test_support {
    use super::*;
...
mod tests {
    ...
    #[tokio::test]
    async fn retired_notebook_transfer_routes_are_unavailable() {
        let dir = tempfile::tempdir().unwrap();
        let store = Arc::new(NotebookStore::new_with_path(dir.path().join("notebook")));
        let app = notebook_router(store);

        for (method, uri) in [
            (Method::POST, "/api/notebook/import"),
            (Method::POST, "/api/notebook/import/preview"),
            (Method::POST, "/api/notebook/import/folder"),
            (Method::GET, "/api/notebook/export.zip"),
            (Method::GET, "/api/notebook/export-vault.zip"),
            (Method::GET, "/api/notebook/entries/missing/export.md"),
        ] {
            let response = app
                .clone()
                .oneshot(
